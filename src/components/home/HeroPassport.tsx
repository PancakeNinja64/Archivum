...
import { fmtInt, fmtRelative, platformLabel, coverageColorVar, bandLabel, commercialUseLabel } from "@/lib/utils";


/** Six-stage strip along the brand arc; documented nodes filled. */
function LineageStrip({ band }: { band: DatasetSummary["coverageBand"] }) {
  ...
  return (
    <svg viewBox="0 0 300 44" className="mt-1 h-11 w-full" aria-hidden>
      ...
      {stages.map((s, i) => {
        ...
        const strong = band === "extensive" || i === 5 || i < 2;
        return (
          <g key={s}>
...
export function HeroPassport({ datasets }: { datasets: DatasetSummary[] }) {
  ...
  if (!d) return null;
  const color = coverageColorVar(d.coverageTotal);

  const rows: [string, React.ReactNode][] = [
    ["License", <span key="l" className={d.license.spdx === "Not stated" ? "text-asserted" : "text-foreground"}>{d.license.spdx}</span>],
    ["Commercial terms", <span key="c" className={d.license.commercialUse === "permitted" ? "text-verified" : d.license.commercialUse === "not_stated" ? "text-asserted" : "text-risk"}>{commercialUseLabel[d.license.commercialUse]}</span>],
    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b border-border px-5 py-3">
          <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Dataset record</span>
          <span className="font-mono text-[11px] text-muted-foreground">{bandLabel[d.coverageBand]}</span>
        </div>
          ...
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="text-right">
                <CountUp to={d.coverageTotal} color={color} run={!reduce && idx === 0} />
                <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">% documented</p>
              </div>
            </div>
            ...
            <motion.div {...stagger(4)}>
              ...
              <LineageStrip band={d.coverageBand} />
            </motion.div>
          </motion.div>
        </AnimatePresence>
      </motion.div>
