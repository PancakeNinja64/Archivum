...
import { fmtDate, evidenceColorVar, evidenceLabel } from "@/lib/utils";
      ...
      <div className="overflow-hidden rounded-[10px] border border-border bg-surface">
          ...
          {ALL_STAGES.slice(0, -1).map((s, i) => {
            ...
            return (
              <path
                ...
                d={d}
                fill="none"
                stroke={gap ? "var(--risk)" : edge ? evidenceColorVar[edge.evidence] : "var(--border-strong)"}
          ...
          {ALL_STAGES.map((s, i) => {
            ...
            return (
              <g
                ...
                tabIndex={0}
                role="button"
                aria-label={`${s.label}, ${evidenceLabel[node.evidence]}`}
              ...
              >
                <circle
                  ...
                  fill="var(--surface)"
                  stroke={evidenceColorVar[node.evidence]}
                  strokeWidth={isActive ? 2 : 1.5}
                  style={{ transition: reduce ? undefined : "r 180ms ease" }}
                />
                {node.evidence === "documented" && <circle cx={x} cy={y} r="3.5" fill={evidenceColorVar.documented} />}
                {node.evidence === "reported" && (
                  <path d={`M ${x} ${y - 3.5} A 3.5 3.5 0 0 1 ${x} ${y + 3.5} Z`} fill={evidenceColorVar.reported} />
                )}
        ...
        <ol className="sr-only">
          {ALL_STAGES.map((s) => {
            ...
            return (
              <li key={s.stage}>
                {s.label}: {n ? `${evidenceLabel[n.evidence]}, ${n.actor}, ${fmtDate(n.timestamp)}` : "undocumented"}
              </li>
            );
          })}
        </ol>
          ...
          {active && (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <EvidenceDot label={active.evidence} />
              </div>
