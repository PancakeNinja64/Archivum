...
export function TerrainField({
  ...
  /** Screen-space point the leader line runs to, in CSS px. */
  ...
  const anchorRef = useRef(cardAnchor);
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      for (const rec of recordsRef.current) {
        ...
        const cur = pres.get(rec.slug) ?? target;
        if (Math.abs(cur - target) < 0.002) {
          if (cur !== target) pres.set(rec.slug, target);
          continue;
        }
        animating = true;
      ...
      camRef.current = cam;
      const wells = buildWells(recordsRef.current, massRef.current, (s) => pres.get(s) ?? 0);
      ...
      if (hoveredWell && reticleRef.current > 0.01) {
        drawReticle(ctx, hoveredWell, wells, cam, tokens, reticleRef.current, anchorRef.current);
      }
    };
