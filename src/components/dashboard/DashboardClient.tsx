...
import { fmtRelative, coverageColorVar } from "@/lib/utils";
...
export function DashboardClient() {
  const [banner, setBanner] = useState(!live);
  ...
  useEffect(() => {
    (async () => {
      if (live) {
        ...
        if (!data?.user) { setSignedIn(false); setWatch([]); setEvents([]); return; }
        setSignedIn(true);
      }
      getWatchlist().then(setWatch).catch(() => setWatch([]));
      getActivity().then(setEvents).catch(() => setEvents([]));
    })();
  }, []);
  ...
  const summary = watch && watch.length
    ? {
        ok: watch.filter((w) => w.licenseStatus === "ok").length,
        review: watch.filter((w) => w.licenseStatus !== "ok").length,
        avg: Math.round(watch.reduce((s, w) => s + w.coverageTotal, 0) / watch.length),
      }
    : null;
    ...
    <div className="mx-auto max-w-7xl px-6 pb-24 pt-28 md:px-8">
      {banner && (
        <div className="mb-8 flex items-center justify-between gap-4 rounded-[10px] border border-accent-soft bg-accent-wash px-5 py-3.5">
          <p className="text-sm text-foreground">
            ...
            <span className="ml-3 text-muted-foreground">Sample data shown for illustration.</span>
          </p>
        ...
        {watch === null ? (
          ...
          <>
            <div className="bg-surface p-5">
              <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Saved</p>
              <p className="tnum mt-1.5 font-mono text-2xl text-foreground">{watch.length}</p>
            </div>
            <div className="bg-surface p-5">
              <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Licence stated</p>
              <p className="tnum mt-1.5 font-mono text-2xl text-verified">{summary?.ok ?? 0}</p>
            </div>
            <div className="bg-surface p-5">
              <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Licence not found</p>
              <p className="tnum mt-1.5 font-mono text-2xl text-asserted">{summary?.review ?? 0}</p>
            </div>
            <div className="bg-surface p-5">
              <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Avg coverage</p>
              <p className="tnum mt-1.5 font-mono text-2xl" style={{ color: summary ? coverageColorVar(summary.avg) : "var(--muted-foreground)" }}>
                {summary ? `${summary.avg}%` : "—"}
              </p>
            </div>
          </>
      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-[1.5fr_1fr]">
        ...
        <section aria-label="Saved datasets">
          <h2 className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Saved datasets</h2>
              ...
              <table className="w-full min-w-[620px] border-collapse text-sm">
                <thead>
                  <tr className="border-b border-border bg-surface text-left font-mono text-[11px] uppercase tracking-[0.12em] text-muted-foreground">
                    ...
                    <th className="px-4 py-3 font-normal">Licence</th>
                  </tr>
                </thead>
                <tbody>
                  {watch === null &&
                    ...
                    <tr key={w.slug} className="border-b border-border last:border-0 hover:bg-accent-wash/50 dark:hover:bg-accent-wash">
                      ...
                      <td className="px-4 py-3 font-mono text-[12px]">
                        <span className={w.licenseStatus === "ok" ? "text-verified" : w.licenseStatus === "changed" ? "text-risk" : "text-asserted"}>
                          {w.licenseStatus === "unresolved" ? "not stated" : w.licenseStatus}
                        </span>
                      </td>
                    </tr>
                  ))}
        ...
        <section aria-label="Recent changes">
          <h2 className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Recent changes</h2>
          {events !== null && events.length === 0 ? (
            <p className="mt-3 rounded-[10px] border border-border bg-surface p-5 text-sm text-muted-foreground">
              No changes recorded for your saved datasets yet.
            </p>
          ) : (
