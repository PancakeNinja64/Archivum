...
export function PublishClient() {
  ...
  // Projected coverage: honest about what a submission alone documents.
  ...
  const validate = (s: number): string[] => {
    ...
    if (s === 3) {
      if (!form.license.trim()) e.push("Name the licence, or write 'Not stated' — an honest unknown reads better than a wrong claim.");
    ...
    <div className="mx-auto max-w-6xl px-6 pb-24 pt-28 md:px-8">
      <header className="max-w-2xl">
        ...
        <p className="mt-4 text-base leading-relaxed text-muted-foreground">
          ...
          the more of the record is documented — and the preview on the right updates as you type.
        </p>
      </header>
          ...
          <div className="overflow-hidden rounded-[10px] border border-border bg-surface">
            ...
            <div className="p-5">
              ...
              <div className="mt-5 flex items-end justify-between border-t border-border pt-4">
                <div>
                  <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Projected starting coverage</p>
                  <p className="mt-1 text-[11px] leading-snug text-muted-foreground">{projected.note}</p>
                </div>
                <span className="tnum font-mono text-4xl" style={{ color: coverageColorVar(projected.composite) }}>{projected.composite}%</span>
              </div>
            </div>
          </div>
