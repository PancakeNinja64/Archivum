...
async function once(url: string, headers: Record<string, string>): Promise<FetchOutcome> {
  ...
  try {
    ...
    if (res.status === 304) {
      return { status: 304, ok: false, notModified: true, etag: res.headers.get('etag'), json: null, text: null };
    }
    ...
    return { status: res.status, ok: res.ok, notModified: false, etag: res.headers.get('etag'), json, text };
  } catch {
    return { status: 0, ok: false, notModified: false, etag: null, json: null, text: null };
  } finally {
    clearTimeout(timer);
  }
}
