...
const toSummary = (d: Dataset): DatasetSummary => ({
  ...
  trustScore: d.trustScore, trustTier: d.trustTier, lastUpdated: d.lastUpdated, version: d.version,
});

export async function getDatasets(f: DatasetFilters = {}): Promise<Paginated<DatasetSummary>> {
  const { page = 1, pageSize = 24, sort = 'trust' } = f;
  ...
  if (f.tier?.length) rows = rows.filter((d) => f.tier!.includes(d.trustTier));
  if (f.commercialOnly) rows = rows.filter((d) => d.license.commercialUse);
  if (typeof f.minTrustScore === 'number') rows = rows.filter((d) => d.trustScore >= f.minTrustScore!);
...
export const getFeatured = (n = 6): Promise<DatasetSummary[]> =>
  // Deliberately mixed: strong scores plus one weak one, so the grading reads as real.
