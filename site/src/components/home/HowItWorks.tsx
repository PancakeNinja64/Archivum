...
import { platformLabel, scoreColorVar } from "@/lib/utils";
...
const steps = [
  {
    ...
    body: "Filter by domain, language, modality, license, and minimum trust score. Stop hunting across a dozen sites.",
  },
  {
    n: "02",
    title: "Verify",
      ...
      <ul className="mt-3 space-y-2">
        {items.slice(0, 4).map((d, i) => (
          ...
          >
            ...
            <span className="tnum font-mono text-sm" style={{ color: scoreColorVar(d.trustScore) }}>{d.trustScore}</span>
          </motion.li>
        ))}
      </ul>
    </div>
  ...
  return (
    <div className="rounded-md border border-border bg-background p-4">
      <div className="flex items-start justify-between gap-3 border-b border-border pb-3">
        ...
        <span className="tnum font-mono text-2xl" style={{ color: scoreColorVar(d.trustScore) }}>{d.trustScore}</span>
      </div>
...
export function HowItWorks({ featured }: { featured: DatasetSummary[] }) {
  ...
  const strong = featured.find((d) => d.trustScore >= 85) ?? featured[0];
