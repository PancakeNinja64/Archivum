...
export function MarketplacePreview({ initial }: { initial: DatasetSummary[] }) {
  ...
  useEffect(() => {
    ...
    getDatasets({
      ...
      minTrustScore: minScore || undefined,
      pageSize: 6,
      sort: "trust",
    }).then((r) => {
      if (!live) return;
      ...
      if (!modality && minScore === 0) {
        const low = initial.find((d) => d.trustScore < 50);
        if (low && !items.some((d) => d.trustScore < 50)) items = [...items.slice(0, 5), low];
      }
      ...
      <div className="mx-auto max-w-6xl px-6 md:px-8">
        ...
        <div className="mt-10 flex flex-wrap items-center gap-3">
          <div className="flex flex-wrap gap-2" role="group" aria-label="Filter by modality">
            <button
              ...
              className={`rounded-md border px-3 py-1.5 font-mono text-[12px] transition-colors ${modality === null ? "border-accent-strong text-accent-strong dark:text-accent" : "border-border text-muted-foreground hover:text-foreground"}`}
            >
              all
            </button>
            {MODALITIES.map((m) => (
              <button
                ...
                className={`rounded-md border px-3 py-1.5 font-mono text-[12px] transition-colors ${modality === m ? "border-accent-strong text-accent-strong dark:text-accent" : "border-border text-muted-foreground hover:text-foreground"}`}
              >
                {m}
              </button>
            ))}
          </div>
          <label className="ml-auto flex items-center gap-3 font-mono text-[12px] text-muted-foreground">
            ...
            <input
              ...
              className="w-36 accent-[var(--accent-strong)]"
            />
          </label>
        </div>
        ...
        {empty && (
          <div className="mt-8 rounded-[10px] border border-border bg-surface p-10 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Try lowering the minimum trust score.</p>
          </div>
        )}
